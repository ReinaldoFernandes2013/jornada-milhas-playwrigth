...
export default class PaginaPrincipal {
  ...
  async definirPassageirosAdultos(quantitade: number) {
    for (let i = 1; i < quantitade; i++) {
        await this.botaoIncrementarAdultos.click();
      }
    }

    async definirPassageirosCriancas(quantitade: number) {
    for (let i = 0; i < quantitade; i++) {
        await this.botaoIncrementarCriancas.click();
      }
    }

    async definirPasseirosBebes(quantitade: number) {
    for (let i = 0; i < quantitade; i++) {
    ...
    async buscarPassagen() {
      await this.botaoBuscarPassagens.click();
    }

    async estaMostrandoPassagem(
      tipoTrageto: 'Somente ida' | 'Ida e volta',
      origem: string,
      destino: string,
    ) {
        await expect(this.textoIdaVolta).toHaveText(tipoTrageto);
